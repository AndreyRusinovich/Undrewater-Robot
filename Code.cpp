...
void rotateTo(double &yts, bool &line, int &i, double &dth) {                              // С 49 по 71 строку функция для поворота по курсу линии
	if (!line) {
		for (auto obj : mur.getDetectedObjectsList(0)) {
...
void findCirc(bool &line) {                                             // С 73 по 87 строку функция для поиска круга
	if (line) {
		for (auto obj : mur.getDetectedObjectsList(0)) {
...
void findBlack(bool &black) {                                       // С 90 по 103 строку функция для поиска чёрного квадрата
...
int main() {
	...
	while (1) {
		...
		findBlack(black);          // функция для поиска чёрного квадрата      
		findCirc(line);         // функция для поиска круга
	}
	return 0;
}
